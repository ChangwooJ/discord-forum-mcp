...
export function registerListForumThreads(server: McpServer) {
  server.tool(
    "list_forum_threads",
    "포럼 채널의 스레드 목록을 최신순으로 반환한다. 활성 스레드가 먼저 오고 그다음 아카이브 스레드가 온다",
    {
      ...
      archivedBefore: z
        ...
        .optional()
        .describe(
          "이 ISO 타임스탬프보다 이전에 아카이브된 스레드부터 가져온다. 이전 응답의 nextArchivedBefore를 그대로 넣으면 다음 페이지가 된다"
        ),
    },
    async ({ channelId, limit = DEFAULT_LIMIT, includeArchived = true, archivedBefore }) => {
      ...
      const nextArchivedBefore =
        hasMoreArchived && lastArchived?.archivedAt
